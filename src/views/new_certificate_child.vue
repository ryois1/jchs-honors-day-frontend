    ...
    <div id="newcert">
        <b-container fluid>
            <b-row>
                ...
                <b-col><h3>You have <b-badge>{{this.certs_remaining}}</b-badge> out of <b-badge>{{this.max_certs}}</b-badge> certificates remaining</h3></b-col>
            </b-row>
            <b-row>
                <b-col>
                    ...
                    <b-form @submit.prevent="processForm">
                        ...
                        <div v-for="index in input_index" :key="index">
                            <b-container fluid>
                                <b-row>
                                    <b-col>
                                        <b-form-group id="student_id-group" label="Student ID or Name:" label-for="student_id">
                                        <b-form-input id="student_id" v-model="student_id[index-1]" placeholder="Student ID or Name" required></b-form-input>
                                        </b-form-group>
                                    </b-col>
                                    <b-col>
                                        <h4>Actions</h4>
                                        <b-button variant="danger" @click="deleteRow(index-1)">Remove</b-button>    <b-button :disabled="!isNaN(student_id[index-1])" variant="primary" @click="lookupStudent(index-1)">Lookup</b-button>
                                            <b-form-group  v-for="student in lookedup_student" :key="student">
                                                <b-form-radio v-model="student_id[index-1]" :value="student.id">{{`${student.first_name} ${student.last_name} (${student.id})`}}</b-form-radio>
                                            </b-form-group>
    ...
    methods: {
        addStudent(){
            if(this.max_certs == this.current_certs_count){
                this.$parent.$toast.error('No more certificate slots.', { position: 'top-right' });
            }else{
                this.input_index++;
        ...
        lookupStudent: async function(index){
            ...
            if (await authToken == null){
                ...
                const { data } = await axios.post(`${vm.$parent.API_BASE_URL}/students/search`,loookup_data,{
                    headers: {
